use std::{
	collections::HashMap,
	io::{self, ErrorKind},
	path::PathBuf, process::{Output, Stdio},
};

use tokio::{
	fs::{File, OpenOptions},
	io::{AsyncReadExt, AsyncWriteExt},
	net::TcpStream, process::Command,
};
...
pub trait Config {
	...
	async fn execute_cgi(&self, request: &Request, stream: &mut TcpStream, raw_left: &mut str) -> Result<(Output, String), io::Error> {
	// execute cgi :
		//		- check program path
		...
		if request.path().is_file() == false { return Err(io::Error::new(ErrorKind::NotFound, "CGI failure: not found")) }

		let file = request.path();
		let executor = self.cgi().get(file.extension().unwrap().to_str().unwrap()).unwrap();
		...
		if output.status.success() == false {
			return Err(io::Error::new(ErrorKind::Other, format!("CGI failure: program exited with status {}", output.status)))
		}
	...
	fn cgi_envs(&self, request: &Request) -> HashMap<String, String> {
		...
		env.insert("HTTP_CONNECTION".to_owned(), if request.keep_connection_alive() == true { "keep-alive" } else { "close "}.to_owned());
...
mod utils {
	...
	use tokio::{io::{AsyncReadExt, AsyncWriteExt}, net::TcpStream, process::Child};

	use crate::request::Request;

	pub async fn send_body_to_cgi(request: &Request, stream: &mut TcpStream, child: &mut Child, raw_left: &mut str) -> Result<String, io::Error> {

		if let Some(mut stdin) = child.stdin.take() {
			if request.content_length().is_none() { return Ok(raw_left.to_owned()) }

			let content_len = request.content_length().unwrap().to_owned();

			if raw_left.len() >= content_len {
				unsafe {
					stdin.write_all(raw_left[..content_len].as_bytes_mut()).await?;
				}
			...
			return Err(io::Error::new(ErrorKind::BrokenPipe, "failed to establish connection with CGI's pipe"))
		}
	}
	...
	pub fn extract_boundary(content_type: Option<&String>) -> Option<String> {
		...
		if content_type.is_none() { return None }

		let content_type = content_type.unwrap();
		if content_type.contains(to_find) == false { return None }

		let boundary_pos = content_type.find(to_find).unwrap() + to_find.len();
		if content_type.len() < boundary_pos { return None }
